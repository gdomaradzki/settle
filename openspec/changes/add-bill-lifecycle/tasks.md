...
- [ ] Install `@trpc/server`, `@trpc/client`, `@trpc/react-query`, `@trpc/next`, `@tanstack/react-query`, `superjson`, `zod` (if not already present).
...
- [ ] Create `src/server/root-router.ts`:
  - `import 'server-only';`
...
- [ ] Create `src/features/bills/schemas.ts`:
...
- [ ] Create `src/features/bills/errors.ts`:
...
- [ ] Create `src/features/bills/bill-service.ts`:
  - `import 'server-only';`
...
- [ ] Implement `createBill(input: CreateBillInput, actorId: string): Promise<Bill>`:
  - Inserts Bill with status DRAFT, line items, and a `created` BillEvent.
- [ ] Implement `updateBill(input: UpdateBillInput, actorId: string): Promise<Bill>`:
...
- [ ] Implement `submitBill(billId: string, actorId: string): Promise<Bill>`:
...
- [ ] Implement `approveBill(billId: string, actorId: string): Promise<Bill>`:
...
- [ ] Implement `rejectBill(billId: string, actorId: string, reason: string): Promise<Bill>`:
  - Loads actor. If `actor.role !== 'APPROVER'`, throws `UnauthorizedError`.
  - Source must be PENDING_APPROVAL. Throws `InvalidTransitionError` otherwise.
  - Sets `status = REJECTED`, stamps `rejectedAt`, `rejectedReason`. Writes `rejected` event with `reason` in payload.
- [ ] Implement `scheduleBill(billId: string, actorId: string, payDate: Date, method: PaymentMethod): Promise<Bill>`:
...
- [ ] Implement `payBill(billId: string, actorId: string): Promise<Bill>`:
...
- [ ] Implement `listBills(input: ListBillsInput, actorId: string)`:
...
- [ ] Register `billRouter` under `bills` in `src/server/root-router.ts`.
...
- [ ] Create `scripts/verify-lifecycle.ts` that calls, in sequence:
...
- [ ] Run the verify script. All seven steps pass.
- [ ] Re-seed the DB (`npm run db:seed`) to reset state for subsequent changes. Confirm the seed still runs idempotently.
- [ ] `npm run build` passes without TypeScript errors.
- [ ] `npm run dev` boots; hitting `/api/trpc/bill.list` via curl returns a valid JSON response with the seeded bills.
