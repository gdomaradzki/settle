...
- [x] In `src/features/bills/bill-service.ts`, extract a private `payBillInner(tx: Tx, billId: string, actorId: string): Promise<Bill>` helper containing the body of the existing `payBill` (source-status check, confirmation generation, bill update, `paid` BillEvent).
- [x] Refactor the public `payBill` to be a thin wrapper that calls `db.$transaction((tx) => payBillInner(tx, billId, actorId))`. No behavioral change visible to callers.
- [x] Verify no other call site changes are required (tRPC `bill.pay`, "Send payment" button still call the public `payBill`).

## Service — System user

- [x] Create `src/features/users/system-user.ts`:
  - `import 'server-only';`
...
- [x] In `prisma/seed.ts`, add an `upsert` for the System user immediately after the existing user seed block:
...
- [x] Confirm the existing `users.list` tRPC query returns the System user along with Gus and Ada (no code change required; the assertion is for verification).

## Service — the cron job

- [x] Create `src/features/payments/process-scheduled-payments.ts`:
  - `import 'server-only';`
...
- [x] In `src/server/cron/registry.ts`, append `processScheduledPayments` to the `cronRegistry` array. Import path: `@/features/payments/process-scheduled-payments`.
- [x] Confirm the file's diff is exactly two lines: one `import` and one array entry. The registry's structure SHALL NOT change.
...
- [x] `npm run db:seed` — confirms the System user is created without errors and is idempotent.
...
- [x] Idempotency check: click "Run now" a second time without scheduling another bill. The JobResult shows `processed: 0`.
- [x] Date filter check: schedule a bill for *tomorrow*. Click "Run now" today. The bill's status remains `SCHEDULED` (not picked up).
- [x] Manual payment still works: schedule a bill, then click "Send payment" on the bill detail page. The bill transitions to PAID via the existing manual flow with the human actor (not the System user).
- [x] `openspec validate --strict add-scheduled-payment-execution` passes.
