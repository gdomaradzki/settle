...
- [x] Create a Vercel project and link the local repo: `vercel link`.
- [x] From Vercel Dashboard → Storage, install the Neon integration (Vercel Postgres Marketplace). This provisions a Neon database and auto-injects `DATABASE_URL`, `DATABASE_URL_UNPOOLED`, and related vars into the Vercel project.
- [x] Sync env vars locally: `vercel env pull .env.local`.
- [x] Create `.env.example` at repo root documenting required vars:
...
- [x] Add `.env.local` and `.env` to `.gitignore` (check scaffold default).

## Prisma setup

- [x] Install dependencies: `prisma`, `@prisma/client`.
- [x] Run `npx prisma init` to scaffold `prisma/` and `schema.prisma`.
- [x] Configure `datasource db` in `schema.prisma`:
  ```prisma
  datasource db {
    provider  = "postgresql"
...
- [x] Define `UserRole` enum: `SUBMITTER | APPROVER`.
- [x] Define `User` model with: `id (cuid)`, `name`, `email (unique)`, `role`.

## Schema — `vendors` capability

- [x] Define `PaymentMethod` enum: `ACH | CHECK`.
- [x] Define `Vendor` model with: `id`, `name`, `email?`, `paymentMethod (default ACH)`, `achAccountLast4?`, `achRoutingLast4?`, `mailingAddress?`, `defaultGlCategory?`, `createdAt`.

## Schema — `bills` capability

- [x] Define `BillStatus` enum: `DRAFT | PENDING_APPROVAL | APPROVED | SCHEDULED | PAID | REJECTED`.
- [x] Define `Bill` model with:
...
- [x] Define `BillLineItem` model with: `id`, `billId (FK, cascade)`, `description`, `amountCents`, `type (LineItemType, default EXPENSE)`, `glCategory?`.
- [x] Define `LineItemType` enum: `EXPENSE | ITEM`.
- [x] Define `BillEvent` model with: `id`, `billId (FK, cascade)`, `type (String)`, `actorId (String)`, `payload (Json?)`, `createdAt`, index `@@index([billId, createdAt])`.

## Prisma client singleton

- [x] Create `src/server/db.ts` exporting an HMR-safe Prisma client singleton.
...
- [x] Add to `package.json`: `"prisma": { "seed": "tsx prisma/seed.ts" }`.
- [x] Add dev dependency: `tsx`.

## Verification

- [x] Confirm `.env.development.local` contains non-empty `DATABASE_URL` and `DATABASE_URL_UNPOOLED`.
- [x] Run `npm run db:push` (`prisma db push`). All tables created in Neon, no errors.
- [x] Run `npm run db:seed` (`prisma db seed`). No errors.
- [x] Run `npx prisma studio`. Manually confirm:
...
- [x] Re-run `npm run db:seed`. It succeeds without violating FK constraints (idempotency check).
- [x] Deploy to Vercel: `vercel --prod`. Confirm the build succeeds and the app boots (a later change will add pages; this one only needs the build to pass).
