      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          ...
          <span
            className={cn(
              "font-medium",
              invalidCount > 0 ? "text-red-600" : "text-muted-foreground",
            )}>
            ...
            {rows.map((row, i) => (
              <tr
                ...
                <td
                  className={cn(
                    "px-3 py-2.5",
                    row.status === "invalid" && !row.resolved && "text-red-600",
                  )}>
                  {row.raw.vendor_name || "—"}
                </td>
                ...
                <td className="px-3 py-2.5 text-right tabular-nums">
                  {row.resolved
                    ? formatUSD(row.resolved.amountCents)
                    : row.raw.amount || "—"}
                </td>
                <td className="px-3 py-2.5 text-muted-foreground">
                  {row.raw.due_date || "—"}
                </td>
                <td className="px-3 py-2.5 text-muted-foreground truncate max-w-32">
                  {row.raw.line_description || "—"}
                </td>
                <td className="px-3 py-2.5">
                  {row.errors.length > 0 && (
