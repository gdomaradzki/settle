...
export function SchedulePaymentDialog({ bill, open, onOpenChange, onConfirm, isPending }: Props) {
  const [date, setDate] = useState<Date>(defaultDate);
  const [calOpen, setCalOpen] = useState(false);
  const [method, setMethod] = useState<PaymentMethod>(bill.vendor.paymentMethod as PaymentMethod);
      ...
      <DialogContent className="max-w-sm">
        ...
        <div className="space-y-5 py-2">
          ...
          <div className="space-y-2">
            ...
            {isPast && <p className="text-xs text-destructive">Select a future date.</p>}
          </div>
            ...
            <div className="flex gap-4">
              {(['ACH', 'CHECK'] as PaymentMethod[]).map((m) => (
                <label key={m} className="flex items-center gap-2 cursor-pointer">
                  ...
                  />
                  <span className="text-sm">{m === 'ACH' ? 'ACH' : 'Check'}</span>
                </label>
              ))}
            </div>
          </div>
