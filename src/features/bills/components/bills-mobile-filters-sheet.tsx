...
export function BillsMobileFiltersSheet() {
  ...
  const activeCount = [filters.status, filters.due, filters.mine, filters.q, filters.vendor]
    .filter(Boolean).length;

  return (
    <Sheet>
      <SheetTrigger
        className={cn(buttonVariants({ variant: 'outline', size: 'sm' }), 'gap-1.5 md:hidden')}
      >
